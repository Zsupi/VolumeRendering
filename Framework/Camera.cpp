...
void Camera::Update() {
	...
	model = glm::mat4(1.0f);
	glm::mat4 eInv = glm::translate(camera_position);
	rayDir = glm::inverse((projection * view) * eInv);
...
void Camera::ProcessMessage(unsigned char key, KeyStatus status) {
	if (status == KeyStatus::KEYDOWN) {
		if (key == 'w')
			activeButtons.wPressed = true;
		if (key == 's')
			activeButtons.sPressed = true;
		if (key == 'a')
			activeButtons.aPressed = true;
		if (key == 'd')
			activeButtons.dPressed = true;
		if (key == 'e')
			activeButtons.ePressed = true;
		if (key == 'q')
			activeButtons.qPressed = true;
	}
	if (status == KeyStatus::KEYUP) {
		if (key == 'w')
			activeButtons.wPressed = false;
		if (key == 's')
			activeButtons.sPressed = false;
		if (key == 'a')
			activeButtons.aPressed = false;
		if (key == 'd')
			activeButtons.dPressed = false;
		if (key == 'e')
			activeButtons.ePressed = false;
		if (key == 'q')
			activeButtons.qPressed = false;
	}
}

void Camera::Animate(float dt) {
	if (camera_mode == FREE) {
		if (activeButtons.wPressed)
			camera_position_delta += camera_direction * speed;
		if (activeButtons.sPressed)
			camera_position_delta -= camera_direction * speed;
		if (activeButtons.aPressed)
			camera_position_delta -= glm::cross(camera_direction, camera_up) * speed;
		if (activeButtons.dPressed)
			camera_position_delta += glm::cross(camera_direction, camera_up) * speed;
		if (activeButtons.qPressed)
			camera_position_delta += camera_up * speed;
		if (activeButtons.ePressed)
			camera_position_delta -= camera_up * speed;
	}
}
...
void Camera::GetRayDir(glm::mat4& RayDir) {
	RayDir = rayDir;
}
